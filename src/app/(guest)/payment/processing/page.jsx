  ...
  return (
    <div className="flex-grow flex flex-col items-center pt-20 pb-32 relative z-10 px-6 animate-fade-in">
      <div className="text-center mb-12">
        <p className="text-[10px] tracking-[0.4em] uppercase text-[#8b6f66] mb-4">
          Checkout
        </p>
        <h1
        ...
        <p className="max-w-2xl mx-auto text-sm text-[#5f463d] leading-relaxed">
      ...
      <div className="w-full max-w-xl bg-[#ede8e0]/90 backdrop-blur-md border border-[#e6d1c7] p-12 sm:p-20 rounded-[40px] shadow-2xl text-center relative overflow-hidden animate-fade-up">
        <div className="flex flex-col items-center">
          ...
          <h2
            className="text-3xl sm:text-4xl text-[#3a221c] mb-6"
            style={{ fontFamily: "var(--font-playfair)" }}
          >
            Securing Your Ritual{dots}
          </h2>
          <p className="text-sm text-[#5f463d] leading-relaxed max-w-sm mx-auto mb-10">
            Our artisanal banking gateway is currently validating your
            selection. Please do not refresh the ritual chamber.
          </p>
          ...
          <div className="flex gap-2">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className={`w-1.5 h-1.5 rounded-full bg-[#3a221c] transition-all duration-300 ${dots.length > i ? "opacity-100 scale-125" : "opacity-20 scale-100"}`}
              ></div>
            ))}
          </div>
        </div>
        ...
        <div className="flex gap-4">
          {[
            ...
            <div key={idx} className="flex items-center gap-2">
              ...
              <span className="text-[8px] tracking-[0.2em] uppercase opacity-60">
                {item.label}
              </span>
            </div>
          ))}
      ...
      <Suspense
        fallback={
          <div className="flex-grow flex items-center justify-center min-h-[60vh]">
            <div className="w-8 h-8 border-4 border-[#3a221c]/20 border-t-[#3a221c] rounded-full animate-spin"></div>
          </div>
        }
      >
        <ProcessingContent />
