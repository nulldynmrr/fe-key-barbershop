    ...
    <div className="flex h-screen bg-[#fafafa] font-sans">
      ...
      <aside className="w-64 bg-white border-r border-[#e6d1c7] flex flex-col flex-shrink-0">
        <div className="flex items-center justify-center h-20 px-6 mb-4">
          <div className="relative w-48 h-12">
            <Image src="/images/logo-navbar.png" alt="Key Barber" fill className="object-contain" />
          </div>
        </div>
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`flex items-center group relative px-3 py-3 text-sm font-medium transition-colors ${
                  isActive 
                    ? "text-[#4a1a1a]" 
              ...
              >
                <Icon
                  className={`mr-3 h-5 w-5 flex-shrink-0 ${
                    isActive ? "text-[#4a1a1a]" : "text-[#d8c8bc] group-hover:text-[#8b6f66]"
                  }`}
                  aria-hidden="true"
                />
                <span className={isActive ? "font-bold" : "font-medium"} style={{ fontFamily: "var(--font-plus-jakarta)" }}>
                  {item.name}
                </span>
                {isActive && (
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-20 flex items-center justify-between px-8 bg-[#fafafa]">
          <div>
            <div className="flex items-center text-xs text-[#8b6f66] mb-1" style={{ fontFamily: "var(--font-be-vietnam)" }}>
              <span>Pages</span>
              <span className="mx-2">/</span>
              <span className="capitalize">{pathname.split('/').pop().replace('-', ' ')}</span>
            </div>
            <h1 className="text-2xl font-bold text-[#4a1a1a]" style={{ fontFamily: "var(--font-noto-serif)" }}>
              <span className="capitalize">{pathname.split('/').pop().replace('-', ' ')}</span>
            </h1>
          </div>
